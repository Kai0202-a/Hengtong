  ...
  try {
    ...
    for (const d of dealers) {
      ...
      if (!u) {
        ...
        await usersCol.insertOne({ username, passwordHash: hash, role: 'dealer', status: 'active', company, createdAt: new Date() });
        imported++;
      } else {
        const set = {};
        ...
        if (Object.keys(set).length) { await usersCol.updateOne({ _id: u._id }, { $set: set }); updated++; } else { skipped++; }
      }
    }
