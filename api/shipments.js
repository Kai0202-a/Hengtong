      ...
      if (summary === 'true') {
        ...
        if (groupBy === 'company' || groupBy === 'month') {
          ...
          const pipeline = [
            { $match: match },
            { $project: proj },
            { $group: { _id: groupField, totalQuantity: { $sum: { $ifNull: [ '$quantity', 0 ] } }, totalAmount: { $sum: { $ifNull: [ '$amount', 0 ] } }, totalCost: { $sum: { $cond: [ { $gte: ['$cost', 0] }, { $multiply: ['$cost', { $ifNull: ['$quantity', 0] }] }, { $ifNull: ['$amount', 0] } ] } } } },
            { $sort: { _id: 1 } }
          ];
          const groups = await collection.aggregate(pipeline).toArray();
          const totalsPipeline = [
            { $match: match },
            { $group: { _id: null, totalQuantity: { $sum: { $ifNull: [ '$quantity', 0 ] } }, totalAmount: { $sum: { $ifNull: [ '$amount', 0 ] } }, totalCost: { $sum: { $cond: [ { $gte: ['$cost', 0] }, { $multiply: ['$cost', { $ifNull: ['$quantity', 0] }] }, { $ifNull: ['$amount', 0] } ] } } } }
          ];
